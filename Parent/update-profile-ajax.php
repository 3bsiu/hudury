...
try {
    ...
    $stmt = $pdo->prepare("SELECT Parent_ID FROM parent WHERE Email = ? AND Parent_ID != ?");
    ...
    $stmt = $pdo->prepare("UPDATE parent SET Email = ?, Phone = ? WHERE Parent_ID = ?");
    $stmt->execute([$email, $phone, $parentId]);
    ...
    error_log("Error updating parent profile: " . $e->getMessage());
