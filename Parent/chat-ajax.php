...
if ($action === 'getMessages') {
    try {
        ...
        ");
        $stmt->execute([$currentParentId, $teacherId]);
        $relationship = $stmt->fetch(PDO::FETCH_ASSOC);
        
        if (!$relationship || $relationship['count'] == 0) {
            echo json_encode(['success' => false, 'message' => 'Unauthorized: Teacher not assigned to your child\'s classes']);
            exit();
        }
        ...
        $stmt = $pdo->prepare("
            ...
            FROM conversation
            WHERE ((Participant_1_Type = 'parent' AND Participant_1_ID = ? AND Participant_2_Type = 'teacher' AND Participant_2_ID = ?)
                OR (Participant_1_Type = 'teacher' AND Participant_1_ID = ? AND Participant_2_Type = 'parent' AND Participant_2_ID = ?))
        ");
        $stmt->execute([$currentParentId, $teacherId, $teacherId, $currentParentId]);
    ...
    try {
        ...
        $stmt = $pdo->prepare("
            ...
            VALUES (?, 'parent', ?, 'teacher', ?, ?, 0, NOW())
        ");
        $stmt->execute([$conversationId, $currentParentId, $teacherId, $messageText]);
    ...
    try {
        ...
        if ($conversation) {
            ...
            $stmt = $pdo->prepare("
                ...
                AND Is_Read = 0
            ");
            $stmt->execute([$conversation['Conversation_ID'], $teacherId, $currentParentId]);
        }
